...
class BinomialABTest:
    ...
    def set_design(
        ...
        *,
        ...
        self._planned_looks_cache = [
            (idx, float(t)) for idx, t in enumerate(looks, start=1)
        ]
    ...
    def _planned_looks(self) -> List[Tuple[int, float]]:
        ...
        if raw.empty:
            ...
            self._planned_looks_cache = [
                (int(r["look"]), float(r["planned_t"])) for r in raw.to_dict("records")
            ]
        return self._planned_looks_cache
    ...
    def _state_values(self) -> Dict[str, Any]:
        ...
        info_time_prev = float(
            cache_vals.get("info_time", (prev_nA + prev_nB) / Nmax if Nmax else 0.0)
        )
    ...
    def update(self, payload: Dict[str, int]) -> None:
        ...
        prev_info_time = float(
            state.get("prev_info_time", (prev_nA + prev_nB) / Nmax if Nmax else 0.0)
        )
