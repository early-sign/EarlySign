"""EarlySign DSL Design v22 - sequential ledger read/write demo based on v18.
...
>>> ab = BinomialABTestV22(ledger, labels={"experiment_id": "exp_ab4"})
...
>>> ep = ENormalMixtureV22(eledger, labels={"run": "mixture"})
...
class BinomialABTestV22:
    ...
    def _write_row(self, kind: str, payload: Dict[str, Any]) -> None:
        with self.ledger.session() as sess:
            sess.insert(kind=kind, payload=payload)

    def _info_time(self, snapshot: Dict[str, float], design: Dict[str, Any]) -> float:
        Nmax = design["Nmax"]
        return (snapshot["nA"] + snapshot["nB"]) / Nmax if Nmax else 0.0
    ...
    def update(self, payload: Dict[str, int]) -> None:
        ...
        # 1) observation row (ledger read -> compute delta -> ledger write)
        obs_payload = dict(
            nA=int(payload["nA"]),
            mA=int(payload["mA"]),
            nB=int(payload["nB"]),
        ...
        self._write_row("observation", obs_payload)

        # 2) snapshot row derived from previous snapshot + delta
        nA_now = snapshot_prev["nA"] + obs_payload["nA"]
        ...
        I1 = self._info_time(latest_snapshot, design)
        self._write_row("info", dict(info_time=float(I1)))

        # 4) stat/decision if a look becomes due after this update
        ...
        if due is not None:
            ...
            self._write_row("decision", decision_payload)
...
def _ab_workload() -> BinomialABTestV22:
    con = ibis.duckdb.connect()
