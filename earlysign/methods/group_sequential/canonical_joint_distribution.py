...
class CanonicalJointDistribution:
    ...
    def __init__(self, n_sims: int = 20000, rng_seed: Optional[int] = None): # Default to 20k for balanced speed/accuracy
        self.n_sims = n_sims
        ...
        for i in range(k):
            ...
            if i == 0:
                ...
                def f_a(val):
                    ...
                    return np.mean(stopped_efficacy_prev | (efficacy_curr & ~stopped_futility_prev)) - a_cum[i]
            ...
            if i == k - 1:
                ...
                if i == 0:
                    def f_b(val):
                        ...
                        return np.mean(futility_curr) - b_cum[0]
                else:
                    ...
                    def f_b(val):
                        futility_curr = z_sims_h1[:, i] < val
                        return np.mean(stopped_futility_prev_h1 | (futility_curr & ~stopped_efficacy_prev_h1)) - b_cum[i]
    ...
    def _compute_ros_boundaries(
        ...
        for i in range(K - 1):
            ...
            total_rej_h0 += np.count_nonzero(just_stopped_efficacy_h0)
            ...
            total_fut_h1 += np.count_nonzero(just_stopped_futility_h1)
    ...
    def solve_ros_inflation_factor(
        ...
        def f(ros):
            _, _, tot_acc = self._compute_ros_boundaries(
                K, alpha, beta, rho, ros, z_h0=z_h0, alpha_cum=alpha_cum, beta_cum=beta_cum
            )
            return tot_acc - beta
    ...
    def evaluate_ros_design_characteristics(
        ...
        try:
            ...
            asn_0 = self.evaluate_asn_dual_boundaries(info_times, a, b, drift=0.0, z_base=z_h0)
            asn_05delta = self.evaluate_asn_dual_boundaries(info_times, a, b, drift=0.5 * delta_target, z_base=z_h0)
            asn_delta = self.evaluate_asn_dual_boundaries(info_times, a, b, drift=delta_target, z_base=z_h0)
        finally:
            self.n_sims = n_sims_original
            self._rng = rng_original
