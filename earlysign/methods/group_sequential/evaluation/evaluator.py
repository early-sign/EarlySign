...
class OperatingCharacteristicEvaluator:
    ...
    def evaluate_rejection_probability(
        ...
        return self._cjd.compute_rejection_probability(t, boundaries, drift=drift)
    ...
    def evaluate_rejection_probability_canonical(
        ...
        return self._cjd.compute_rejection_probability(
            info_times, boundaries, drift=drift
        )
    ...
    def evaluate_design_characteristics(
        ...
        boundaries = self._cjd.solve_boundaries_from_spending(
            info_times, spending, tails=tails
        )
        
        # 2. Solve for drift eta (the drift that gives target power)
        eta = self._cjd.solve_drift(
            info_times, boundaries, target_power=target_power, tails=tails
        )
        ...
        # delta = eta
        asn_0 = self._cjd.evaluate_asn(info_times, boundaries, drift=0.0, tails=tails)
        asn_05delta = self._cjd.evaluate_asn(info_times, boundaries, drift=0.5 * eta, tails=tails)
        asn_delta = self._cjd.evaluate_asn(info_times, boundaries, drift=eta, tails=tails)
        asn_15delta = self._cjd.evaluate_asn(info_times, boundaries, drift=1.5 * eta, tails=tails)
        ...
        return {
            ...
            "asn_looks": {
                "0": asn_0,
                "0.5delta": asn_05delta,
                "delta": asn_delta,
                "1.5delta": asn_15delta,
            }
        }
