...
class DesignLookRecord(RecordBase):
    KIND = "design-look"
    MAPPING = {"look": ("look", "int64"), "planned_t": ("planned_t", "float64")}
...
class SnapshotRecord(RecordBase):
    KIND = "snapshot"
    MAPPING = {"nA": ("nA", "float64"), "mA": ("mA", "float64"), "nB": ("nB", "float64"), "mB": ("mB", "float64")}
...
def _build_records(ledger: Ledger) -> Dict[str, RecordBase]:
    return {
        ...
        "design_look": DesignLookRecord(ledger),
...
class BinomialABTest:
    ...
    def _planned_looks(self) -> List[Tuple[int, float]]:
        rows = self._records["design_look"].list(order_by="planned_t")
    ...
    def _latest_snapshot(self) -> Dict[str, float]:
        ...
        snap = self._records["snapshot"].latest(defaults)
    ...
    def _latest_design(self) -> Dict[str, Any]:
        design = self._records["design"].latest(
            {"planned_max_n": 0.0, "alpha": 0.05, "spending_family": "obrien_fleming"}
        )
        return dict(design)
