"""
Framework + Apps: Reader-base refactor; generic latest()/rows_since(); profiling in __main__
===========================================================================================
...
- Reader hierarchy (LedgerReaderBase with generic helpers):
    * base_view()                         : labels-scoped raw rows
    * latest_row(payload_type)            : generic latest row for any type
    * rows_since(payload_type, ts_expr)   : generic rows after/since ts
  Specialized readers build on these generic helpers:
    * ABDesignLatest   : latest design (planned_max_n, ts)
    * ABDesignLooks    : explode looks from the latest design ts
    * ABSnapshotLatest : latest snapshot (or a single zero row if none)
    * ABObsSince       : sum of observations strictly after given ts
    * ABRowsBase       : typed AB rows (for stat/decision inspection)
...
class LedgerReaderBase:
    ...
    # ------- Generic helpers available to ANY payload type -------
...
class BinomialABTest:
    ...
    def update(self, payload: Dict[str, int]) -> None:
        ...
        I1 = (nA + nB) / Nmax  (nA,nB include current delta)
        """
        ...
        with LedgerSession(self.ledger) as s:
            ...
            # Readers (generic helpers underneath)
            ...
            now = before.cross_join(add_tbl).cross_join(Dmax).select(
...
class ENormalMixture:
    ...
    def update(self, *, x: float) -> None:
        ...
        with LedgerSession(self.ledger) as s:
            ...
            # Threshold 1/alpha from the latest e_design (generic latest_row + SQL json_extract)
...
""")
    ...
    # Expose generic helper if needed by callers
    def latest_row(self, payload_type: str) -> ibis.Expr:
