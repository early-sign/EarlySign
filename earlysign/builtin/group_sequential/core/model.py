...
class CanonicalJointModel:
    ...
    def evaluate_asn(
        ...
        if method == "numerical_integration":
            ...
            # Probabilities of stopping at each look i
            p_stop = pu + pl if target_tails == 2 else pu
            # Expected look index (1-indexed)
            looks = np.arange(1, len(t) + 1)
            # Add probability of NOT stopping (reaches K)
            # Actually p_stop should sum to total stopping prob.
            # Boundary solving usually ensures we stop by K if we don't cross?
            # No, if it's non-binding or we just don't cross, we reach K.
            prob_reaches_k = 1.0 - np.sum(p_stop)
        ...
        for i in range(k):
            if target_tails == 2:
                crossing = (np.abs(samples[:, i]) > u[i]) & ~stopped
            else:
                crossing = (samples[:, i] > u[i]) & ~stopped
            stop_looks[crossing] = i + 1
            stopped |= crossing
