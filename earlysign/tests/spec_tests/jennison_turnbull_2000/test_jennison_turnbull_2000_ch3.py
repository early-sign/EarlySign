...
@then(parsers.parse("the maximum information (I_max) should be {expected:f} with {atol:f} precision"))
def then_check_i_max(results, expected, atol):
    assert results["i_max"] == pytest.approx(expected, abs=atol)

@then(parsers.parse("the fixed sample information (I_f) should be around {threshold:f}"))
...
@then(parsers.parse("the standardized boundary at look k should be 2.072 with 0.08 precision * sqrt(5/k)"))
@then(parsers.parse("the standardized boundary at look k should be {c:f} with {atol:f} precision * sqrt({k_total:d}/k)"))
...
@then(parsers.parse("the total sample size (n_max) should be {n_max:f} with {atol:f} precision"))
def then_check_n_max_float(results, n_max, atol):
    assert results["n_max"] == pytest.approx(n_max, abs=atol)

@then(parsers.parse("the boundary values should be around \"{boundaries}\" with {atol:f} precision"))
@then(parsers.parse("the boundary values should be \"{boundaries}\" with {atol:f} precision"))
...
def when_table31_eval(n_actual, design_params, evaluator, planner):
    ...
    alpha_actual = evaluator.evaluate_rejection_probability(
        n=actual_n, boundaries=boundaries, theta=0.0, sigma2=var_diff, trial_type="paired"
    )
    power_actual = evaluator.evaluate_rejection_probability(
        n=actual_n, boundaries=boundaries, theta=theta, sigma2=var_diff, trial_type="paired"
    )
...
@then(parsers.parse("the actual power should be {power:f} with {atol:f} precision for effect 1.0 and variance 4.0"))
...
@then(parsers.parse("the Pocock boundary value (C) should be {c_val:f} with {atol:f} precision"))
...
@then(parsers.parse("the critical difference in proportions should be {diff:f} with {atol:f} precision / sqrt(k)"))
...
@then(parsers.parse("the O'Brien-Fleming boundary constant (C_OBF) should be {c_val:f} with {atol:f} precision"))
...
@then(parsers.parse("the critical difference in proportions should be {diff:f} with {atol:f} precision * sqrt(p_bar * (1-p_bar)) / k"))
...
@given(parsers.parse("a two-sided single-arm binomial test design with {k:d} looks and total sample size {n_max:d}"))
...
def when_evaluate_binomial_oc(design_params, evaluator, cjd):
    ...
    res_plan = DesignPlanner(cjd=cjd).plan_design(
        alpha=0.05, power=0.9, theta=0.2, sigma2=p0*(1-p0), k=k, 
        shape_type="pocock", trial_type="binomial-single", round_to_k=True
    )
    boundaries = res_plan["boundaries"]
    
    # 2. Evaluate alpha (at p=0.6, var=0.24)
    alpha_actual = evaluator.evaluate_rejection_probability(
        n=n_actual, boundaries=boundaries, theta=0.0, sigma2=p0*(1-p0), trial_type="binomial-single"
    )
    
    # 3. Evaluate NOMINAL power (at p=0.8, effect=0.2, using var=0.24)
    power_nominal = evaluator.evaluate_rejection_probability(
        n=n_actual, boundaries=boundaries, theta=0.2, sigma2=p0*(1-p0), trial_type="binomial-single"
    )
    
    # 4. Evaluate TRUE power (at p=0.8, effect=0.2, var=0.16)
    power_true = evaluator.evaluate_rejection_probability(
        n=n_actual, boundaries=boundaries, theta=0.2, sigma2=0.16, trial_type="binomial-single"
    )
...
@then(parsers.parse("the nominal power at p = {p:f} (using null variance) should be {power:f} with {atol:f} precision"))
def then_check_nominal_power(results, p, power, atol):
    assert results["power_nominal"] == pytest.approx(power, abs=atol)

@then(parsers.parse("the true power at p = {p:f} (using alternative variance) should be {power:f} with {atol:f} precision"))
...
@then(parsers.parse("the maximum information (I_max) should be {i_max:f} with {atol:f} precision"))
def then_check_i_max(results, i_max, atol):
    assert results["i_max"] == pytest.approx(i_max, abs=atol)

@then(parsers.parse("the fixed sample information (I_f) should be {i_f:f} with {atol:f} precision"))
def then_check_i_f(results, i_f, atol):
    assert results["i_fixed"] == pytest.approx(i_f, abs=atol)

@then(parsers.parse("the total sample size (n_max) should be {n_max:d} with {atol:f} precision"))
@then(parsers.parse("the total sample size (n_max) should be {n_max:f} with {atol:f} precision"))
def then_check_n_max(results, n_max, atol):
    assert results["n_max"] == pytest.approx(n_max, abs=atol)

@then(parsers.parse("the total number of events (d_max) should be {d_max:d} with {atol:f} precision"))
...
@given(parsers.parse("a two-sided A/B test design with {k:d} looks and sample size per group {n_g:d}"))
...
def when_evaluate_ab_oc(design_params, evaluator, cjd):
    ...
    res_plan = DesignPlanner(cjd=cjd).plan_design(
        alpha=0.05, power=0.8, theta=0.2, sigma2=0.25, k=k,
        shape_type="obrien_fleming", trial_type="binomial-ab", round_to_k=True
    )
    boundaries = res_plan["boundaries"]
    
    # 2. Evaluate alpha (drift=0)
    alpha_actual = evaluator.evaluate_rejection_probability(
        n=n_actual, boundaries=boundaries, theta=0.0, sigma2=0.25, trial_type="binomial-ab"
    )
    
    # 3. Evaluate NOMINAL power (delta=0.2, sigma^2=0.25)
    power_nominal = evaluator.evaluate_rejection_probability(
        n=n_actual, boundaries=boundaries, theta=0.2, sigma2=0.25, trial_type="binomial-ab"
    )
    
    # 4. Evaluate TRUE power (pA=0.4, pB=0.6, variance=0.24)
    power_true = evaluator.evaluate_rejection_probability(
        n=n_actual, boundaries=boundaries, theta=0.2, sigma2=0.24, trial_type="binomial-ab"
    )
...
@then(parsers.parse("the total sample size per group (n_g) should be {n_g:d} with {atol:f} precision"))
@then(parsers.parse("the sample size per group (n_g) should be {n_g:d} with {atol:f} precision"))
def then_check_n_g(results, n_g, atol):
    assert results["n_g"] == pytest.approx(n_g, abs=atol)

@then(parsers.parse("the sample size increment per group per look should be {n:d} with {atol:f} precision"))
def then_check_n_increment(results, n, atol):
    assert results["n_per_look"] == pytest.approx(n, abs=atol)

@then(parsers.parse("the nominal power at delta = {delta:f} (using sigma^2 = {sigma2:f}) should be {power:f} with {atol:f} precision"))
def then_check_power_nominal_ab(results, power, delta, sigma2, atol):
    # Calibrated tolerance from Gherkin
    assert results["power_nominal"] == pytest.approx(power, abs=atol)

@then(parsers.parse("the true power at p_A = {pa:f}, p_B = {pb:f} (using sigma^2 = {sigma2:f}) should be {power:f} with {atol:f} precision"))
...
@given(parsers.parse("a two-sided t-test design with alpha {alpha:f}"), target_fixture="design_params")
...
def when_group_sequential_t_test_eval(design_params, planner, evaluator, cjd):
    ...
    approx = planner.calculate_t_test_power_approx(alpha, i_max_adj, i_fixed_adj, theta, nu_K)
...
@then(parsers.parse("the power approximation (3.20) should be {val:f} with {atol:f} precision"))
def then_check_approx_320(results, val, atol):
    assert results["approx_320"] == pytest.approx(val, abs=atol)

@then(parsers.parse("the power approximation (3.21) should be {val:f} with {atol:f} precision"))
def then_check_approx_321(results, val, atol):
    assert results["approx_321"] == pytest.approx(val, abs=atol)

# --- 3.8.2 Examples ---

@when(parsers.parse("I evaluate the group sequential t-test performance for effect {theta_sigma:f} (sigma units)"), target_fixture="results")
def when_group_sequential_t_test_eval_sigma_units(theta_sigma, design_params, planner, evaluator, cjd):
    alpha = design_params["alpha"]
    k = design_params["k"]
    nu_K = design_params["nu_K"]
    p = design_params["p"]
...
@when(parsers.parse("I evaluate the group sequential t-test performance for effect {val:f} (beta_1) and sigma_sq {sigma2:f}"), target_fixture="results")
def when_evaluate_t_test_covariate(val, sigma2, design_params, evaluator):
    ...
    approx = planner.calculate_t_test_power_approx(alpha, i_max_plan, i_fixed_plan, theta, nu_K)
