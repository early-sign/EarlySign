...
def given_rho_config(
    rho: str, ch7_params: Dict[str, Any], K: Optional[str] = None
) -> None:
...
def when_compute_ros(ch7_params: Dict[str, Any]) -> Dict[str, Any]:
    from earlysign.methods.group_sequential.plan.operating_characteristics.overrunning import (
...
def when_evaluate_asn(ch7_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_asn_percent(drift_val: float) -> float:
        ...
        return float(100.0 * r_ld * oc.asn)
...
def when_evaluate_asn_onesided(ch7_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for _ in range(3):  # 3 iterations at lower precision is enough
        ...
        current_drift = model.solve_drift(
            ...
            target_power=power,
            tails=1,
            futility_boundaries=b.tolist() if b is not None else [],
            method="simulation",
        )
    ...
    def get_asn_percent(drift_val: float) -> float:
        oc = sim.evaluate_point(
            drift=drift_val,
            info_times=info_times,
            upper_boundaries=cast(np.ndarray, a),
            lower_boundaries=cast(np.ndarray, b),
        )
        return float(100.0 * r_os * oc.asn)

    return {
        "R_OS": float(100.0 * r_os),
        "ASN_0.0": float(get_asn_percent(0.0)),
        "ASN_0.5": get_asn_percent(0.5 * drift_h1),
        "ASN_1.0": get_asn_percent(drift_h1),
