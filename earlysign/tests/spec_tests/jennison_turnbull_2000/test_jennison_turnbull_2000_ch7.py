...
@when("I evaluate the expected sample size relative to fixed design", target_fixture="results")
...
@when(parsers.parse("I analyze the Lan-DeMets design from section 7.2.2 with K={k} and rho={rho}"), target_fixture="results")
...
@when(parsers.parse("I evaluate the required effect size for power {power} using K={k} and rho={rho}"), target_fixture="results")
...
@then(parsers.parse("the fixed sample information (I_f) should be {i_f} with {atol:f} precision"))
def then_check_i_f_ch7(results, i_f, atol):
    assert results["i_fixed"] == pytest.approx(float(i_f), abs=atol)

@then(parsers.parse("the fixed sample size per group should be {n} with {atol:f} precision"))
def then_check_n_fixed_ch7(results, n, atol):
    assert results["n_fixed"] == pytest.approx(float(n), abs=atol)

@then(parsers.parse("the maximum information (I_max) should be {i_max} with {atol:f} precision"))
def then_check_i_max_722(results, i_max, atol):
    assert results["i_max"] == pytest.approx(float(i_max), abs=atol)

@then(parsers.parse("the maximum sample size per group should be {n} with {atol:f} precision"))
def then_check_n_max(results, n, atol):
    # Calibrated tolerance from Gherkin
    assert results["n_max"] == pytest.approx(float(n), abs=atol)

@then(parsers.parse("the required effect size (delta) should be {delta} with {atol:f} precision"))
def then_check_delta_required(results, delta, atol):
    assert results["delta"] == pytest.approx(float(delta), abs=atol)

@then(parsers.parse("the inflation factor R_LD should be {r_ld} with {atol:f} precision"))
...
@when(parsers.parse("I perform a trial with actual information sequence {sequence}"), target_fixture="results")
...
@then(parsers.parse("the power at delta {theta} should be {power:f} with {atol:f} precision"))
...
@when(parsers.parse("I perform a trial with group size {n} per stage until I_max {i_max_target} is reached"), target_fixture="results")
def when_perform_overrunning_trial(n, i_max_target, design_params, evaluator):
    ...
    actual_info = []
    i_curr = 0.0
    while i_curr < i_max_planned - 1e-9: # tiny epsilon to avoid floating point issues at terminal
        i_curr += info_step
        actual_info.append(i_curr)
        if len(actual_info) > 100: break
...
@then(parsers.parse("the final maximum information should be {i_max:f} with {atol:f} precision"))
...
@when(parsers.parse("I evaluate Table 7.4 robustness with K_tilde={k}, rho={rho}, r={r}, and pi={pi}"), target_fixture="results")
...
@when(parsers.parse("I evaluate Table 7.5 robustness with K_tilde={k_tilde}, K={k}, and rho={rho}"), target_fixture="results")
...
@when(parsers.parse("I analyze the BHAT trial with calendar months {months}"), target_fixture="bhat_calendar")
...
@then(parsers.parse("the the BHAT boundaries should be \"{b_list}\" with {atol:f} precision"))
...
@then(parsers.parse("the observed Z-statistics {z_list} should reject H0 at look {look}"))
