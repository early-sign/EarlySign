...
class BinomialABTestV20:
    ...
    def update(self, payload: Dict[str, int]) -> None:
        ...
        total_prev = snapshot_prev["nA"] + snapshot_prev["nB"]
        total_now = snapshot_next["nA"] + snapshot_next["nB"]
        Nmax = design["planned_max_n"]
        info_prev = total_prev / Nmax if Nmax else 0.0
        info_now = total_now / Nmax if Nmax else 0.0

        z_value = _pooled_z(snapshot_next["nA"], snapshot_next["mA"], snapshot_next["nB"], snapshot_next["mB"])
        due = next(((idx, t) for idx, t in looks if info_prev < t <= info_now), None)
        boundary = None
        action = None
        if due is not None:
        ...
        with self.ledger.session() as sess:
            ...
            self.info_stage.emit_from_dict(sess, {"info_time": info_now})
            self.stat_stage.emit_from_dict(sess, {"z": z_value})
