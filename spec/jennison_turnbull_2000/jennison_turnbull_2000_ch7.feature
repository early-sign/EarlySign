Feature: Jennison & Turnbull (2000) Chapter 7 (Flexible Monitoring: The Error Spending Approach)
  ...
  Scenario: Group sequential design for parallel two-treatment comparison (Subsection 7.2.2)
    ...
    And the fixed sample size per group (n_f) should be 85 with 1.5 precision
    And the inflation factor R_LD should be 1.075 with 0.01 precision
    And the maximum information (I_max) should be 11.30 with 0.1 precision
    And the maximum sample size per group (n_max) should be 91 with 1.5 precision
  ...
  Scenario: Power evaluation with constrained sample collection budget (Subsection 7.2.2)
    ...
    And the inflation factor R_LD should be 1.075 with 0.01 precision
    And the fixed sample information (I_f) should be 9.30 with 0.05 precision
    And the required effect size (delta) should be 1.06 with 0.02 precision
    When I evaluate the required effect size for power 0.8
    Then the inflation factor R_LD should be 1.081 with 0.01 precision
    And the required effect size (delta) should be 0.92 with 0.01 precision
  ...
  Scenario: Under-running (Subsection 7.2.2). Actual sample collection is slower than planned.
    ...
    Then the power at mu_A - mu_B = ±1 should be 0.88 with 0.02 precision
  ...
  Scenario: Over-running (Subsection 7.2.2). Actual group size is larger than planned.
    ...
    And the power at mu_A - mu_B = ±1 should be 0.92 with 0.02 precision
  ...
  Scenario: Power under mismatched information schedules (Table 7.4)
    ...
    Given a two-sided maximum information test with alpha 0.05
    And the design is planned to attain power 0.9 at effect size delta
    ...
    Then the attained power at effect size delta should be <power> with 0.02 precision
  ...
  Scenario: Beta-Blocker Heart Attack Trial (BHAT) verification (Subsection 7.2.3)
    ...
    Then the boundaries should be "2.53, 2.59, 2.64, 2.50, 2.51, 2.47" with 0.10 precision
  ...
  Scenario: Verification of maximum information test constant R_OS (Table 7.6)
    Given a one-sided maximum information test with alpha 0.05 and power <power> at theta = δ
  ...
  Scenario: Properties of one-sided asymmetric maximum information tests (Table 7.7)
    # Note: This scenario is identical to Table 7.8 except for the target power.
    Given a one-sided maximum information test with alpha 0.05 and power 0.8 at theta = δ
    And a maximum of <K> equally-spaced looks with rho-family spending <rho> for both type-I and type-II errors
  ...
  Scenario: Properties of one-sided asymmetric maximum information tests (Table 7.8)
    ...
    Given a one-sided maximum information test with alpha 0.05 and power 0.9 at theta = δ
    And a maximum of <K> equally-spaced looks with rho-family spending <rho> for both type-I and type-II errors
    When I evaluate the one-sided expected sample size relative to fixed design
    Then the maximum sample size relative to fixed design (R_OS) should be <R_OS_pct> percent with 10.0 precision
    And the expected sample size at theta = 0 should be <ASN_0> percent with 4.0 precision
  ...
  Scenario: Properties of one-sided asymmetric maximum information tests (Table 7.9)
    Given a one-sided maximum information test with alpha 0.05 and power 0.95 at theta = δ
    And a maximum of <K> equally-spaced looks with rho-family spending <rho> for both type-I and type-II errors
    When I evaluate the one-sided expected sample size relative to fixed design
    Then the maximum sample size relative to fixed design (R_OS) should be <R_OS_pct> percent with 10.0 precision
    And the expected sample size at theta = 0 should be <ASN_0> percent with 4.0 precision
