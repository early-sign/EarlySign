...
*   **Complexity Management ($O(N)$ vs $O(N^2)$)**: Replaying the entire history for every new data point is computationally expensive. To counter this, Projections implement the **Incremental Folding** principle via **Intermediate Facts**. 
    Following the principles established in *Versioning in an Event Sourced System* (Young 2017), these intermediate facts (snapshots) are treated as recomputable optimizations rather than the source of truth. They are persisted as facts in the Ledger with a specific `identity` and Ledger's Last Timestamp. If the underlying projection logic or schema changes, the system can discard the intermediate fact and re-fold history from the original event stream to ensure absolute scientific accuracy.
...
1.  **Induction Snapshot**: The Ledger's state ID (Last Timestamp or cumulative hash) representing the observation cutoff.
